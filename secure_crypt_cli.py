...
class SecureCryptCLI:
    ...
    def enforce_passphrase_policy(self, passphrase: str, min_len: int) -> bool:
        if len(passphrase) < min_len:
            console.print(f"[yellow]⚠ Passphrase too short. Minimum {min_len} characters required.[/yellow]")
            return False
        # Basic weak checks
    ...
    def register_flow(self):
        ...
        while True:
            login_pass = Prompt.ask("Create Login Passphrase (min 14 chars)", password=True)
            if self.enforce_passphrase_policy(login_pass, 14):
        ...
        while True:
            recovery_pass = Prompt.ask("Create Recovery Passphrase (min 16 chars)", password=True)
            if recovery_pass == login_pass:
                console.print("[red]Recovery passphrase must be DIFFERENT from login passphrase.[/red]")
                continue
            if self.enforce_passphrase_policy(recovery_pass, 16):
