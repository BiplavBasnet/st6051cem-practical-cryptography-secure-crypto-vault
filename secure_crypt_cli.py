...
from services.app_paths import keys_dir
...
class SecureCryptCLI:
    def __init__(self):
        ...
        self.client_fingerprint = self._build_client_fingerprint()

    @staticmethod
    def _build_client_fingerprint() -> str:
        """Create stable local client fingerprint for scoped lockout controls."""
        ...
        platform_hint = f"{sys.platform}|{os.name}"
        raw = f"{host}|{user}|{platform_hint}".encode()
    ...
    @staticmethod
    def _secure_write_json(path: Path, payload: dict, mode: int = 0o600):
    ...
    @staticmethod
    def _safe_key_path(username: str, purpose: str) -> Path:
        if purpose not in {"auth", "signing", "encryption"}:
            raise ValueError("Invalid key purpose")
        base = keys_dir().resolve()
        user_dir = (base / username).resolve()
        if not str(user_dir).startswith(str(base) + os.sep):
    ...
    def ensure_unlocked(self, purpose: str) -> Optional[bytes]:
        ...
        while True:
            ...
            is_locked, lock_msg, remaining = self.api.check_lockout(
            ...
            with Status("[bold green]Unlocking Identity...", console=console):
                try:
                    p_path = self._safe_key_path(self.current_user['username'], purpose)
                    with open(p_path, "r", encoding="utf-8") as f:
                        bundle = json.load(f)
                    
                    priv_bytes = LocalKeyManager.unlock_key_from_bundle(bundle, passphrase)
                    if priv_bytes:
                        self.api.record_attempt(
                            self.current_user['username'],
                            "login",
                            True,
                            client_fingerprint=self.client_fingerprint,
                        ...
                        for other in ['auth', 'signing', 'encryption']:
                            if other != purpose:
                                o_path = self._safe_key_path(self.current_user['username'], other)
    ...
    def register_flow(self):
        ...
        with Status("[bold green]Generating RSA-3072 Identity Keys...", console=console):
            api_bundle = {}
            local_keys_dir = (keys_dir() / username).resolve()
            self._secure_mkdir(local_keys_dir, 0o700)
            ...
            for purp in ["auth", "signing", "encryption"]:
                ...
                p_path = local_keys_dir / f"{purp}_key.pem"
                self._secure_write_json(p_path, protected_bundle, mode=0o600)
    ...
    def login_flow(self):
        ...
        if not ok_u:
            self.show_error(msg_u)
            return

        try:
            auth_bundle_path = self._safe_key_path(username, "auth")
        except Exception as e:
        ...
        with Status("[bold green]Unlocking Identity...", console=console):
            try:
                with open(auth_bundle_path, "r", encoding="utf-8") as f:
                    content = f.read()
                ...
                # PKI Auth via API
                success, user, shared_secret, msg = self.api.login_user(
                    username,
                    priv_key_data=priv_bytes,
                    client_fingerprint=self.client_fingerprint,
                )
                ...
                if success:
                    ...
                    for purp in ['signing', 'encryption']:
                        p_path = self._safe_key_path(username, purp)
                        with open(p_path, "r", encoding="utf-8") as f:
    ...
    def recovery_flow(self):
        ...
        with Status("[bold yellow]Attempting Emergency Decryption...", console=console):
            try:
                ...
                for purp in ["auth", "signing", "encryption"]:
                    k_path = self._safe_key_path(username, purp)
                ...
                for path, priv_bytes in new_protected_files.items():
                    ...
                    self._secure_write_json(Path(path), new_bundle, mode=0o600)
    ...
    def add_secret_flow(self):
        ...
        if "@" in username:
            v2, m2 = CryptoUtils.validate_input(username, 'email')
        else:
            v2, m2 = CryptoUtils.validate_input(username, 'general')
        if not v1: self.show_error(m1); return
        if not v2: self.show_error(m2); return
    ...
    def view_secret_flow(self):
        ...
        with Status("[bold yellow]Decrypting Secret...", console=console):
            res, msg = self.api.decrypt_secret(
                self.current_user['id'],
                int(entry_id),
                priv_key_data=priv_key
            )

        if msg == "Success" and res:
            ...
            if totp:
                console.print(f"[bold cyan]TOTP Secret:[/bold cyan] {totp}")
        ...
        if choice == "1":
            ...
            if Confirm.ask("Proceed with rotation?"):
                ...
                with Status("[bold green]Rotating...", console=console):
                    try:
                        api_bundle = {}
                        local_keys_dir = (keys_dir() / self.current_user['username']).resolve()
                        self._secure_mkdir(local_keys_dir, 0o700)
                        ...
                        for purp in ["auth", "signing", "encryption"]:
                            ...
                            # Save locally
                            p_path = local_keys_dir / f"{purp}_key.pem"
                            self._secure_write_json(p_path, p_bundle, mode=0o600)
                        ...
                        if success:
                            ...
                            for p in ["auth", "signing", "encryption"]:
                                ...
                                p_path = local_keys_dir / f"{p}_key.pem"
                                with open(p_path, "r", encoding="utf-8") as f:
